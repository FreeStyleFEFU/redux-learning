...
export const SliderNavigation: FC<SliderNavigationProps> = (props) => {
        ...
        () => [
            {
                ...
                action: goToPrev,
                dataId: `${dataID}-prev`,
                isVisible: !isSmart || (!isContentFitting && reachedEdge !== ReachedEdge.Start),
            },
            {
                direction: 'next',
                action: goToNext,
                dataId: `${dataID}-next`,
                isVisible: !isSmart || (!isContentFitting && reachedEdge !== ReachedEdge.End),
            },
        ],
    ...
    useEffect(() => {
        ...
        const handleProgress = (swiper1: SwiperClass) => {
            setContentFitting(swiper1.wrapperEl.clientWidth >= swiper1.wrapperEl.scrollWidth);
        ...
        <div className={clsx(styles.navigation, classes?.root)}>
            {buttons.map((button) => {
                ...
                return (
                    ...
                    >
                        <ArrowRightIcon className={clsx(styles.navigationIcon, classes?.icon)} />
                    </button>
                );
            })}
        </div>
