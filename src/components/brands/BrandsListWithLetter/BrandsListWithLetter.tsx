...
export const BrandsListWithLetter: FC<AllBrandsBlockProps> = (props) => {
    ...
    () =>
      Array.from(
        new Set(
          items
            .map((item) =>
              item.name.length > 0 ? item.name[0].toUpperCase() : null
            )
  ...
  const navigationAutoScroll = (letter: string) => {
    const activeNavLetterElement = document.querySelector(
      `[data-nav-letter="${letter}"]`
    );

    if (
      activeNavLetterElement === null ||
      navListRef === null ||
  ...
  const onChangeActiveNavLetter = (letter: string) => {
    ...
    const brandLetter = document.querySelector(
  ...
  const windowScrollHandler = () => {
    ...
    const brandLettersElements = document.querySelectorAll(
      "[data-brand-letter]"
    );
